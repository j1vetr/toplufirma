...
export default function FaturaDetay() {
  ...
  function odemeKaydet() {
    ...
    createOdeme.mutate({
      ...
      onSuccess: () => {
        ...
        setOdemeModal(false); setOdemeTutar(""); toast({ title: "Odeme kaydedildi" });
      },
  ...
  function durumGuncelle(durum: string) {
    updateFatura.mutate({ id, data: { durum } }, {
      onSuccess: () => { qc.invalidateQueries({ queryKey: getGetFaturaQueryKey(id) }); toast({ title: "Durum guncellendi" }); },
      onError: () => toast({ title: "Hata", variant: "destructive" }),
    });
  }
  ...
  if (!fatura) return <div className="text-center py-16 text-muted-foreground">Fatura bulunamadi.</div>;
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">{fatura.cariAd} {fatura.gemiAd ? `- ${fatura.gemiAd}` : ""}</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="rounded-full" onClick={() => window.print()}><Printer className="mr-1 h-4 w-4" /> Yazdir</Button>
          {(fatura.durum === "acik" || fatura.durum === "kismi_odendi") && (
            <Button size="sm" className="rounded-full" onClick={() => setOdemeModal(true)} data-testid="button-odeme-ekle">
              <Plus className="mr-1 h-4 w-4" /> Odeme Kaydet
            </Button>
          )}
        </div>
      </div>
      ...
      {fatura.odemeler && fatura.odemeler.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-base">Odeme Kayitlari</CardTitle></CardHeader>
          <CardContent>
      ...
      <Dialog open={odemeModal} onOpenChange={setOdemeModal}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={odemeBankaId} onValueChange={setOdemeBankaId}>
                <SelectTrigger data-testid="select-odeme-banka"><SelectValue placeholder="Secin (opsiyonel)" /></SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOdemeModal(false)} className="rounded-full">Iptal</Button>
