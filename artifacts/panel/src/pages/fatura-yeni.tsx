...
export default function FaturaYeni() {
  ...
  const { data: filtrelenmisCariler = [] } = useListFirmalar(
    catiFirmaId ? { tip: "bagli", catiFirmaId: Number(catiFirmaId) } : { tip: "bagli" },
    { query: { queryKey: [...getListFirmalarQueryKey(), "bagli", catiFirmaId] } },
  );
  ...
  const seciliCatiFirma = catiFirmalar.find(f => f.id === Number(catiFirmaId));
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4">
          ...
          {/* Bağlı firma + Grup firma — readonly when gemi selected, editable fallback otherwise */}
          {gemiSecildi ? (
            <>
              <div className="space-y-1.5">
            ...
            <>
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={v => {
                    setBagliFirmaId(v);
                    const musteri = filtrelenmisCariler.find(f => f.id === Number(v));
            ...
            return (
              <div key={i} className="grid grid-cols-12 gap-2 items-center" data-testid={`kalem-${i}`}>
                <KalemAciklamaInput
                  ...
                  data-testid={`input-kalem-aciklama-${i}`}
                />
                {isOzel ? (
                  <Input
